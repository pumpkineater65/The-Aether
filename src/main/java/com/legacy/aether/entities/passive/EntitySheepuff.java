...
public class EntitySheepuff extends AnimalEntity {
    ...
    private static final Map<DyeColor, float[]> COLORS = Maps.newEnumMap((Map<DyeColor, float[]>)Arrays.stream(DyeColor.values()).collect(Collectors.toMap((dyeColor_1) -> dyeColor_1, EntitySheepuff::getDyedColor)));
    ...
    public EntitySheepuff createChild(ServerWorld world, PassiveEntity entity) {
    ...
    private DyeColor getChildColor(AnimalEntity entity, AnimalEntity mate) {
        ...
        return optionalItem.map(DyeItem.class::cast).map(DyeItem::getColor).orElseGet(() -> {
            return this.world.random.nextBoolean() ? parentColor : mateColor;
        });
    }
